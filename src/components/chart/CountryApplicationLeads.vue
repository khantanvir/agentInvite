...
export default {
  ...
  data: function () {
    return {
      ...
      mapLoading: false,
      isChanged: false,
      nightMode: this.themeMode,
    }
  },
  computed: {
    themeMode() {
      return JSON.parse(localStorage.getItem('darker'))
    },
  },
  watch: {
    themeModeData: {
      ...
      Request.GET_REQ(`/country-application-leads-reports/${this.selectcountryId}`)
        .then((res) => {
          for (let i = 0; i < res.data.length; i++) {
            ...
            this.series.push(item.status_leads_count && parseInt(item.status_leads_count))
      ...
      await Request.GET_REQ(`/country-application-leads-reports/${this.selectcountryId}`)
        .then((res) => {
          for (let i = 0; i < res.data.length; i++) {
            ...
            this.chartOptions.fill.colors.push(item.status_color)
            this.chartOptions.colors.push(item.status_color)
            this.series.push(parseInt(item.status_leads_count))
          }
          this.chartOptions = {
            ...this.chartOptions,
            ...{
