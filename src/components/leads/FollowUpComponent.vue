<template>
  <div class="card no-outer-spacing">
    <div id="headingTwo2" class="card-header">
      <section class="mb-0 mt-0">
        <div
          role="menu"
          class="collapsed"
          data-toggle="collapse"
          data-target="#withoutSpacingAccordionTwo"
          aria-expanded="false"
          aria-controls="withoutSpacingAccordionTwo"
        >
          Follow up
          <div class="icons">
            <svg
        ...
        <div class="row">
          <div class="col col-md-12">
            <div class="lms-pagination">
              <base-pagination simple v-model="page" :links="pageLinks" @paginate="followUpInfo" />
            </div>
          </div>
        </div>
      </div>
...
const follow_up = ref('')
// follow_up_date_time: "",
const followup_id = ref('')
const page = ref(1)
const totalFollowUps = ref(0)
const errors = ref({})
const followUps = ref([])
const lazyLoadFollowup = ref(false)
const editFollow = ref(false)
const follow_up_date_time = ref(new Date())
const timezone = ref('')
const isFollowUpDoneId = ref('')

//methods
function handleFollowUp() {
  // lazyLoadFollowup = true;
  const data = {
    lead_id: props.leadId,
    user_id: user.value.id,
    follow_up: follow_up.value,
    follow_up_date_time: follow_up_date_time.value,
  }
...
function followUpInfo() {
  Request.GET_REQ(`/follow-up-info/${props.leadId}?page=${page.value}`)
    .then((res) => {
...
function totalFollowup() {
  Request.GET_REQ(`/follow-up-info-total/${props.leadId}`)
    .then((res) => {
      totalFollowUps.value = res.data.length
    })
    .catch((err) => {
      Notify(err.res.data)
    })
...
function editFollowup(id) {
  Request.GET_REQ(`/follow-up-info-edit/${id}`)
    .then((res) => {
      editFollow.value = true
      follow_up.value = res.data.follow_up
      followup_id.value = id
      follow_up_date_time.value = moment(res.data.follow_up_date_time).format()
    })
    .catch((err) => {
      Notify(err.res.data)
    })
...
function handleUpdateFollowUp() {
  ...
  let data = {
    follow_up: follow_up,
    follow_up_date_time: new Date(follow_up_date_time).toISOString(),
  }

  Request.POST_REQ(data, `update-follow-up/${followup_id.value}`)
    .then(() => {
      lazyLoadFollowup.value = false
      follow_up.value = ''
      editFollow.value = false
      emitter.emit('followupReload')
      store.dispatch('followUpLists')
...
function isFollowUpDone(followUpId) {
  $('#followupConfirmationModal').modal('show')
  isFollowUpDoneId.value = followUpId
}
function followUpComplete(status) {
  Request.GET_REQ(`/follow-up-complete-status/${status}/${isFollowUpDoneId.value}`)
    .then(() => {
      emitter.emit('followupReload')
