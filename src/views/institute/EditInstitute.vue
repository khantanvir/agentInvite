                ...
                <div class="row d-flex align-items-center">
                  ...
                  <div class="col col-md-4">
                    <div v-if="institute_logo" class="office-logo">
                      <img alt="Institute Logo" :src="selectedLogo" />
                    </div>
                  </div>
                </div>
              </div>
                  ...
                  <div class="col col-md-12 text-right">
                    <div class="row ml-4">
                      <div v-show="index == contacts.length - 1">
                        <button
                          class="btn btn-warning mb-2 mr-2 btn-rounded inline-flex"
                          @click.prevent="add"
                        >
                          Add More
                          <svg
                      ...
                      <div v-show="index || (!index && contacts.length > 1)">
                        <button
                          class="btn btn-danger mb-2 mr-2 btn-rounded inline-flex"
...
async function fetchPrimary() {
  ...
  try {
    const res = await Request.GET_REQ(`/institute/${id}`)
    represent_country_id.value = res.data.represent_country_id
    institute_name.value = res.data.institute_name
    ...
    funds_requirements_for_visa.value = res.data.funds_requirements_for_visa
    application_fee.value = res.data.application_fee
    ...
    institute_benifits.value = res.data.institute_benifits
    per_time_work_details.value = res.data.per_time_work_details
    scholarship_policy.value = res.data.scholarship_policy
    institute_important_notes.value = res.data.institute_important_notes
    institute_logo.value = res.data.institute_logo
    selectedLogo.value = baseURL.baseURL + res.data.institute_logo
    institute_prospectus.value = res.data.institute_prospectus
    institute_course_pdf.value = res.data.institute_course_pdf
...
async function handleSubmit() {
  ...
  try {
    const res = await Request.POST_REQ(data, `/update-institute/${id}`)
    isLoading.value = false
...
function getErrors(index, field) {
  let property = `contacts.${index}.${field}`
  return errors.value[property]
}
function onFileChange(event) {
  store.dispatch('baseurl', '')
  institute_logo.value = null
  ...
  if (file.size > 1048770) {
    ...
    reader.onload = (event) => {
      ...
      selectedLogo.value = event.target.result
    }
