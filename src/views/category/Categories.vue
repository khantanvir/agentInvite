                  ...
                  <table class="table table-bordered">
                    ...
                    <tbody>
                      <tr v-for="(category, index) in categories.data" :key="index">
                        ...
                        <td>
                          <div class="text-left">
                            <span :class="category.active === 1 ? 'text-success' : 'text-danger'">{{
...
function handleSubmit() {
  Request.POST_REQ({ category_name: category_name }, '/add-category')
    .then(() => {
      category_name.value = ''
      emitter.emit('reload')
    ...
    .catch((err) => {
      ...
      errors.value.category_name && Notify.error(errors.value.category_name[0])
    })
}
...
async function allCategories() {
  try {
    const res = await Request.GET_REQ(`/categories?page=${page.value}`)
...
async function totalCategories() {
  try {
    const res = await Request.GET_REQ(`/total-categories`)
...
function handleEdit(id) {
  ...
  if (catid.value > 0 && isEdit.value == true) {
    Request.GET_REQ(`/edit-category/${catid.value}`)
      .then((response) => {
...
function handleUdate() {
  Request.POST_REQ({ category_name: category_name.value }, `/update-category/${catid.value}`)
    .then(() => {
      category_name.value = ''
      emitter.emit('reload')
      Notify.success('Category Updated...')
